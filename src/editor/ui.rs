...
use crate::ui::layout::{self, Axis, Justify, Layout};
    ...
    pub fn new() -> anyhow::Result<Editor> {
        ...
        let insert = ui::Box::new((
            ...
            ui::Elem::new(INSERT_LINE)
                .style(|style| style.justify(Justify::Stretch).overflow(true)),
        ));

        let complete_selector = ui::Box::new(
            ui::Elem::new(COMPLETE_SELECTOR).style(|style| style.axis(Axis::Vertical)),
        )
        .tag(TAG_COMPLETE_SELECTOR)
        .style(|style| style.axis(Axis::Vertical).hidden(true));

        let path_selector = ui::Box::new((
            ui::Box::new(
                ui::Elem::new(PATH_LINE)
                    .style(|style| style.justify(Justify::Stretch).overflow(true)),
            ),
            ui::Box::new((
                ui::Elem::new(PATH_SELECTOR.0)
                    .style(|style| style.axis(Axis::Vertical).justify(Justify::Stretch)),
        ...
        let error = ui::Box::new(ui::Elem::new(ERROR)).tag(TAG_ERROR);

        let mut table = ui::Table::default();
        let mut map = Vec::new();

        (insert, complete_selector, path_selector, command, error).walk(
            &mut layout,
            &mut table,
            &mut map,
            root,
...
const PROMPT: ElementImpl = ElementImpl {
    ...
    render: |shell, layout, current, mut term| {
        ...
        debug_assert!(
            usize::from(current.x)
                + strip_str(shell.prompt.as_str())
                    .map(|s| s.width())
                    .sum::<usize>()
...
const INSERT_LINE: ElementImpl = ElementImpl {
    info: |shell| {
        ...
        let cursor_len = shell.editor.command.is_empty().then_some(s0_len);
    ...
    render: |shell, layout, current, mut term| {
        ...
        if let Some(cmd) = shell.ast {
            ...
            queue!(term, style::Print(shell.editor.insert.as_str()),)?;
        }
        ...
        if !suggest.is_empty() {
            ...
            let color = config
                ...
                .suggest
                .color()
                .unwrap_or(style::Color::DarkGrey);
            queue!(
                term,
...
const MODE: ElementImpl = ElementImpl {
    info: |shell| {
        shell.editor.mode.str().map(|s| layout::SpaceInfo {
            length: s.width() + 1,
            cursor: None,
        })
    },
...
const COMMAND_LINE: ElementImpl = ElementImpl {
    info: |shell| {
        ...
        let (s0, s1) = shell
            .editor
            .command
            .split(shell.editor.command.cursor().end);
        let s0_len = s0.width();
        let s1_len = s1.width();
    ...
    render: |shell, layout, current, mut term| {
        let hint = match shell.editor.mode {
            editor::Mode::Normal | editor::Mode::Visual | editor::Mode::PathSelector => true,
...
const TIPS: ElementImpl = ElementImpl {
    info: |shell| {
        shell.editor.ready.map(|c| layout::SpaceInfo {
...
const PATH_LINE: ElementImpl = ElementImpl {
    info: |shell| {
        matches!(shell.editor.mode, editor::Mode::PathSelector).then_some(())?;
...
const PATH_SELECTOR: (ElementImpl, ElementImpl, ElementImpl) = {
    ...
    fn render(
        ...
        for (n, (hint, entry)) in list.take(layout.size.1.into()).enumerate() {
            ...
            let color = if hint {
                style::Colors::new(style::Color::Black, color.unwrap_or(style::Color::White))
            } else {
                style::Colors::new(color.unwrap_or(style::Color::White), style::Color::Reset)
            };
    ...
    const fn imp<const N: u8>() -> ElementImpl {
        ElementImpl {
            ...
            render: |shell, layout, current, term| render(shell, layout, current, term, N),
        }
    }
...
const COMPLETE_SELECTOR: ElementImpl = ElementImpl {
    info: |shell| {
        matches!(shell.editor.mode, editor::Mode::CompleteSelector).then_some(())?;
    ...
    render: |shell, layout, current, mut term| {
            ...
            .take(selector.window.len())
        {
            for (column, comp) in chunk.iter().enumerate() {
                ...
                let (pad, desc_limit) = rem
                    .checked_sub(desc_width)
                    .map(|pad| (pad, None))
        ...
        debug_assert_eq!(current.y + selector.window.len() as u16, layout.range.end.y);
...
const ERROR: ElementImpl = ElementImpl {
    ...
    render: |shell, layout, current, mut term| {
        ...
        debug_assert_eq!(current.x + err.width() as u16, layout.range.end.x);
