...
impl Editor {
    ...
    pub fn step(&mut self, env: &RefCell<Environment>, event: Event) -> anyhow::Result<Action> {
        match (self.mode, event) {
            ...
            (
                ...
                match (self.mode, self.command.first(), code) {
                    // command input
                    (_, _, KeyCode::Char('\r')) => (),
                    (_, Some(_), KeyCode::Char(c)) => self.command.push(c),
                    (_, Some(_), KeyCode::Backspace) => self.command.backspace(),
                    (_, Some(_), KeyCode::Delete) => self.command.delete(),
                    (_, Some(_), KeyCode::Left) => self.command.move_left(),
                    (_, Some(_), KeyCode::Right) => self.command.move_right(),
                    (_, Some(_), KeyCode::Esc) => self.command.clear(),
                    (Mode::Normal, Some(':'), KeyCode::Enter) => match self.command.as_str() {
            ...
            (
                ...
                match (self.mode, self.ready.take(), code) {
                    ...
                    // normal and visual
                    (_, None, KeyCode::Char('i')) => self.mode = Mode::Insert,
                    (Mode::Normal | Mode::Visual, None, KeyCode::Char('a')) => {
                        self.insert.move_right();
                        self.mode = Mode::Insert;
                    }
                    (Mode::Normal, None, KeyCode::Char('v')) => self.mode = Mode::Visual,
                    (Mode::Visual, None, KeyCode::Char('v')) => self.mode = Mode::Normal,
                    ...
                    (Mode::Visual, None, KeyCode::Char('d')) => {
                        self.insert
                            .replace_str_inclusive("", Some(&mut self.clipboard));
                        self.mode = Mode::Normal;
                    }
                    ...
                    // gh
                    (Mode::Normal | Mode::Visual, None, KeyCode::Char('g')) => {
                        self.ready = Some('g')
                    }
                    ...
                    // path selector
                    (Mode::PathSelector, None, KeyCode::Char('j')) => self.path_selector.down()?,
                    (Mode::PathSelector, None, KeyCode::Char('k')) => self.path_selector.up()?,
                    (Mode::PathSelector, None, KeyCode::Char('h')) => self.path_selector.left()?,
                    (Mode::PathSelector, None, KeyCode::Char('l')) => self.path_selector.right()?,
                    ...
                    (Mode::PathSelector, None, KeyCode::Char('y')) => {
                        if let Some(path) = self.path_selector.selected() {
                            ...
                            let path = if !path.is_empty() { path } else { "." };
                            self.clipboard.clear();
    ...
    pub fn mode_switch<T: TermTarget>(
        &mut self,
        prev_mode: Mode,
